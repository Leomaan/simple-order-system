...
export const receiveWebhook = asyncHandler(async (req, res) => {
  try {
    ...
    if (req.headers['x-signature']) {
      ...
      if (!isValid) {
        console.warn(`[Webhook Forbidden] Falha na verificação de assinatura do webhook vindo de ${req.ip}`);
      ...
      console.warn(`[Webhook Forbidden] Requisição sem assinatura em produção vinda de ${req.ip}`);
      return res.status(403).json({ success: false, message: 'Assinatura ausente e obrigatória em produção.' });
    }
  } catch (err) {
    console.error('Erro na validação de assinatura do webhook:', err.message);
