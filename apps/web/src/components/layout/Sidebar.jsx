...
export default function Sidebar({ active, onNavigate, role }) {
  ...
  return (
    <>
      {/* Mobile Top Bar */}
      <div className="md:hidden w-full h-16 bg-neutral-900 border-b border-neutral-800 flex items-center justify-between px-4 shrink-0 sticky top-0 z-40">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-orange-500 to-orange-600 text-white flex items-center justify-center font-black text-base shadow-lg shadow-orange-500/10">
            S
          </div>
          <div>
            <h1 className="text-white font-bold text-sm tracking-tight">Simple Order</h1>
            <p className="text-neutral-550 text-[9px] uppercase font-semibold tracking-wider">{panelLabel}</p>
          </div>
        </div>
        <button
          onClick={() => setIsOpen(!isOpen)}
          className="w-10 h-10 rounded-xl bg-neutral-800/60 border border-neutral-750 flex items-center justify-center text-neutral-400 hover:text-white transition-all cursor-pointer active:scale-95"
        >
      ...
      {/* Sidebar aside */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-neutral-900 border-r border-neutral-800 flex flex-col justify-between shrink-0 select-none transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${
        ...
        <div className="flex flex-col flex-1">
          ...
          <div className="p-6 border-b border-neutral-800 flex items-center gap-3">
            ...
            <div>
              <h1 className="text-white font-bold text-sm tracking-tight">Simple Order</h1>
              <p className="text-neutral-550 text-[10px] uppercase font-semibold mt-0.5 tracking-wider">{panelLabel}</p>
            </div>
            {/* Close button inside aside on mobile */}
            <button 
              onClick={() => setIsOpen(false)} 
              className="md:hidden ml-auto p-1.5 text-neutral-400 hover:text-white hover:bg-neutral-800 rounded-lg"
            >
              <X size={16} />
            </button>
          </div>
